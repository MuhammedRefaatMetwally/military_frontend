...
function getQuarterMonths(q: SalesAnalysisQuarter) { return q.months; }
...
function MetricsCells({
  metrics, maxNet, maxInvoices, barColor,
}: {
  metrics: RollupMetrics; maxNet: number; maxInvoices: number; barColor: string;
}) {
  return (
    <>
      <td style={{ ...analyticsTdBaseStyle("center"), position: "relative" }}>
        <AnalyticsBarCellContent value={metrics.net} max={maxNet} color={barColor} text={formatNumber(metrics.net)} />
      </td>
      {metrics.netYoyPrior != null ? (
        <td style={{ ...analyticsTdBaseStyle("center"), position: "relative" }}>
          <AnalyticsBarCellContent value={metrics.netYoyPrior} max={maxNet} color={barColor} text={formatNumber(metrics.netYoyPrior)} />
        </td>
      ) : (
      ...
      <td style={analyticsTdBaseStyle("center")}><MetricTrend value={metrics.yoy} /></td>
      <td style={analyticsTdBaseStyle("center")}><MetricTrend value={metrics.mom} /></td>
      <td style={{ ...analyticsTdBaseStyle("center"), position: "relative" }}>
        <AnalyticsBarCellContent value={metrics.invoices} max={maxInvoices} color={barColor} text={formatNumber(metrics.invoices)} />
      </td>
      <td style={analyticsTdBaseStyle("center")}>
...
function AccordionLabelCell({
  label, subtitle, depth, isOpen, canExpand, onToggle,
}: {
  label: string; subtitle?: string; depth: 0 | 1 | 2;
  isOpen?: boolean; canExpand?: boolean; onToggle?: () => void;
}) {
  const paddingRight = 12 + depth * 22;
  return (
    <td style={{
      ...analyticsTdBaseStyle("right"), paddingRight, fontSize: 11,
      ...
      {canExpand ? (
        <button type="button" onClick={onToggle}
          ...
          style={{ border: 0, background: "transparent", padding: 0, cursor: "pointer", color: "inherit", font: "inherit" }}
        >
          <ChevronDown size={14} style={{ transition: "transform 160ms ease", transform: isOpen ? "rotate(0deg)" : "rotate(90deg)" }} />
          <span>{label}</span>
          {subtitle && <span style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 500 }}>{subtitle}</span>}
        </button>
      ) : (
        <div className="inline-flex items-center gap-1.5">
          <span style={{ width: 5, height: 5, borderRadius: 999, background: "var(--text-muted)", display: "inline-block", opacity: 0.65 }} />
          <span>{label}</span>
          {subtitle && <span style={{ color: "var(--text-muted)", fontSize: 10, fontWeight: 500 }}>{subtitle}</span>}
        </div>
      )}
...
function SkeletonRows() {
  return (
    <>
      {[...Array(4)].map((_, i) => (
        <tr key={i} style={{ background: i % 2 === 0 ? "var(--bg-elevated)" : undefined }}>
          {[...Array(7)].map((__, j) => (
            <td key={j} style={analyticsTdBaseStyle(j === 0 ? "right" : "center")}>
              <div style={{
...
const DetailedTimeBasedSalesAnalysis = () => {
  ...
  // ── Data fetching ─────────────────────────────────────────────────────────
  const { data: rawData, isLoading, isError } = useDetailedTimeSales(params, {
    staleTime: 5 * 60 * 1000,
  });
  ...
  const { maxNet, maxInvoices } = useMemo(() => {
    ...
    salesAnalysisData.forEach((year) => {
      ...
      year.quarters.forEach((quarter) => {
        ...
        quarter.months.forEach((month) => allMetrics.push(getMonthRollup(month)));
      });
    });
    return {
  ...
  return (
    ...
    >
      <AnalyticsTable headers={headers} minWidth={980}>
        ...
        {isError && !isLoading && (
          <tr>
            <td colSpan={7} style={{ textAlign: "center", padding: 24, color: "var(--accent-red)", fontSize: 12 }}>
        ...
        {!isLoading && !isError && salesAnalysisData.map((year) => {
          ...
          return (
            <Fragment key={year.year}>
              <tr style={{ background: "var(--bg-elevated)" }}>
                <AccordionLabelCell label={year.year} subtitle="السنة" depth={0} canExpand isOpen={isYearExpanded} onToggle={() => toggleYear(year.year)} />
                <MetricsCells metrics={yearMetrics} maxNet={maxNet} maxInvoices={maxInvoices} barColor="#3b82f6" />
              </tr>

              {isYearExpanded && year.quarters.map((quarter) => {
                ...
                return (
                  <Fragment key={quarterKey}>
                    <tr style={{ background: "color-mix(in srgb, var(--bg-elevated) 55%, transparent)" }}>
                      <AccordionLabelCell label={quarter.label} subtitle="ربع سنوي" depth={1} canExpand isOpen={isQuarterExpanded} onToggle={() => toggleQuarter(quarterKey)} />
                      <MetricsCells metrics={quarterMetrics} maxNet={maxNet} maxInvoices={maxInvoices} barColor="#0891b2" />
                    </tr>

                    {isQuarterExpanded && quarter.months.map((month) => (
                      <tr key={`${quarterKey}-${month.id}`}>
                        <AccordionLabelCell label={month.label} subtitle={month.monthOrderLabel} depth={2} />
                        <MetricsCells metrics={getMonthRollup(month)} maxNet={maxNet} maxInvoices={maxInvoices} barColor="#047857" />
                      </tr>
                    ))}
                  </Fragment>
                );
